...
def count_photos(image_folder, photo_extensions=['.jpg']):
    count = 0
    for _, _, files in os.walk(image_folder):
        for file in files:
            if any(file.endswith(ext) for ext in photo_extensions):
...
def generate_narration(api_key, image_folder,photo_count):
